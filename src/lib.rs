...
fn parse_buildrer_struct(
    ...
    for attr in attributes_form_syn(attrs)? {
        match  attr {
...
let fields = fields
    .named
    .into_iter()
    .map(|f|match attributes_form_syn(f.attrs){
...
fn attributes_form_syn(attrs: Vec<syn::Attribute>) -> MultiResult<Vec<BuilderAttribute>>{
    use syn::parse2;
